...
* **`kustomize`:** Declares output target namespaces and injects custom overlays (e.g., `overlays/pvc.yaml` or direct, type-safe CUE structures under `kustomize.overlays` mapped to `overlays/mxc-overlays.yaml`).
